/**
 * Taaaac Core Client - Sincronizzazione Feature Flags & Configurazione Tenant per Barberly
 */
...
const ALL_DEV_ADDONS = [
  ...
  TAAAAC_ADDONS.WHATSAPP_REMINDERS,
];

const DEFAULT_DEV_CONFIG: TenantConfig = {
  id: "dev-tenant",
  nomeAttivita: "Barberly Grooming Club (Dev)",
  sottodominio: "barberly",
  modulo: "barberly",
  piano: "PRO",
  stato: "ATTIVO",
  dataScadenza: null,
...
export async function fetchTenantConfig(domain?: string): Promise<TenantConfig> {
  ...
  try {
    ...
    console.warn(`[TaaaacClient] Barberly fallback per (${coreApiUrl}):`, err?.message || err);
  }
