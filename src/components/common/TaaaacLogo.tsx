...
export function TaaaacIcon({ className = "w-8 h-8", size }: TaaaacIconProps) {
  return (
    ...
    >
      {/* Scintilla / Schiocco energetico (Verde Smeraldo) */}
      <g stroke="#059669" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
        ...
        {/* Stella centrale dello schiocco */}
        <path
      ...
      {/* Mano a tratto continuo (Blu Cobalto) */}
  ...
  return (
    <div className={`inline-flex items-center gap-2 min-w-0 ${className}`}>
      ...
      <div className="flex items-center gap-1.5 min-w-0">
        ...
        {showBadge && badgeText && (
          <span className="text-[9px] uppercase font-bold px-1.5 py-0.5 rounded-md bg-amber-50 text-amber-800 border border-amber-200 shrink-0 tracking-wider">
            {badgeText}
          </span>
        )}
      </div>
